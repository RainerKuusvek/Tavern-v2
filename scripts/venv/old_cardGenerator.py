...
if __name__ == "__main__":
    ...
    # fonts
    titleSize = 68;
    raceclassTextSize = 55;
    heroDescriptionTextSize = 57;
    seasonsDescriptionTextSize = 68
        ...
        for i in range(int(cards[card, 6])):  # for every copy of each card
            ...
            if cards[card, 2] == 'peeve':
                ...
                for nt in range(len(text)):
                    I1.text(np.add(peeveTitleCoords, [0, nt * 20 * 0.9]), text[nt], fill=(10, 10, 10),
                            font=font_peeveTitle)

                # Description
                text = textwrap.wrap("{}".format(cards[card, 3]), width=int(peeveDescriptionWidth))
                peeveTextSquishFactor = 0.7
                for nt in range(len(text)):
                    I1.text(np.add(peeveDescriptionCoords, [0, nt * peeveDescriptionTextSize * peeveTextSquishFactor]),
                            text[nt], fill=(10, 10, 10), font=font_peeveDescription)
            ...
            if cards[card, 2] == 'townsfolk':
                ...
                for nt in range(len(text)):
                    I1.text(np.add(titleCoords, [0, nt * titleSize * shiftFactor]), text[nt], fill=(10, 10, 10),
                            font=font_name)

                # Description
                text = textwrap.wrap("{}".format(cards[card, 4]), width=int(townsfolkDescriptionWidth))
                for nt in range(len(text)):
                    I1.text(np.add(townsfolkDescriptionCoords, [0, nt * heroDescriptionTextSize * shiftFactor * 1.2]),
                            text[nt], fill=(10, 10, 10), font=font_townsfolkDescription)
                ...
                if cards[card, 5] == 'mission modifier':
                    I1.text([rage2Coords[0], rage2Coords[1] + font_raceclass.getsize('TYPE: ')[1]], 'Mission Modifier',
                            fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(missionmod, (
                    xCoord, int(rage2Coords[1] + 0.75 * font_raceclass.getsize('TYPE: ')[1] - iconSize[1] / 2)),
                                  missionmod)  # add icon
                if cards[card, 5] == 'auction':
                    I1.text([rage2Coords[0], rage2Coords[1] + font_raceclass.getsize('TYPE: ')[1]], 'Auction  ',
                            fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(auction, (
                    xCoord, int(rage2Coords[1] + 0.75 * font_raceclass.getsize('TYPE: ')[1] - iconSize[1] / 2)),
                                  auction)  # add icon
                if cards[card, 5] == 'attack':
                    I1.text([rage2Coords[0], rage2Coords[1] + font_raceclass.getsize('TYPE: ')[1]], 'Attack  ',
                            fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(attack, (
                    xCoord, int(rage2Coords[1] + 0.75 * font_raceclass.getsize('TYPE: ')[1] - iconSize[1] / 2)),
                                  attack)  # add icon
                if cards[card, 5] == 'defence':
                    I1.text([rage2Coords[0], rage2Coords[1] + font_raceclass.getsize('TYPE: ')[1]], 'Defence  ',
                            fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(defence, (
                    xCoord, int(rage2Coords[1] + 0.75 * font_raceclass.getsize('TYPE: ')[1] - iconSize[1] / 2)),
                                  defence)  # add icon
                if cards[card, 5] == 'soother':
                    I1.text([rage2Coords[0], rage2Coords[1] + font_raceclass.getsize('TYPE: ')[1]], 'Soother  ',
                            fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(soother, (
                    xCoord, int(rage2Coords[1] + 0.75 * font_raceclass.getsize('TYPE: ')[1] - iconSize[1] / 2)),
                                  soother)  # add icon
                if cards[card, 5] == 'activator':
                    I1.text([rage2Coords[0], rage2Coords[1] + font_raceclass.getsize('TYPE: ')[1]],
                            'Ability Activator  ', fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(activator, (
                    xCoord, int(rage2Coords[1] + 0.75 * font_raceclass.getsize('TYPE: ')[1] - iconSize[1] / 2)),
                                  activator)  # add icon
                if cards[card, 5] == 'anticriminal':
                    I1.text([rage2Coords[0], rage2Coords[1] + font_raceclass.getsize('TYPE: ')[1]], 'Anticriminal  ',
                            fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(anticriminal, (
                    xCoord, int(rage2Coords[1] + 0.75 * font_raceclass.getsize('TYPE: ')[1] - iconSize[1] / 2)),
                                  anticriminal)  # add icon
                ...
                if cards[card, 7] == '1':
                    ...
                    cardPNG.paste(neutral, (xCoord + 120, raceCoordsTownsfolk[1] + raceclassTextSize),
                                  neutral)  # add icon
            ...
            if cards[card, 2] == 'Quest':
                ...
                if cards[card, 9] == 'any':
                    ...
                    cardPNG.paste(any, (coords[0] + iconSize[0] + margin[0], coords[1] + raceclassTextSize),
                                  any)  # add icon
                if cards[card, 9] == 'warrior':
                    I1.text([coords[0] + raceLen, coords[1]], ', WARRIOR', fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(warrior, (coords[0] + iconSize[0] + margin[0], coords[1] + raceclassTextSize),
                                  warrior)  # add icon
                if cards[card, 9] == 'bandit':
                    I1.text([coords[0] + raceLen, coords[1]], ', BANDIT', fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(bandit, (coords[0] + iconSize[0] + margin[0], coords[1] + raceclassTextSize),
                                  bandit)  # add icon
                if cards[card, 9] == 'priest':
                    I1.text([coords[0] + raceLen, coords[1]], ', PRIEST', fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(priest, (coords[0] + iconSize[0] + margin[0], coords[1] + raceclassTextSize),
                                  priest)  # add icon
                if cards[card, 9] == 'troubadour':
                    I1.text([coords[0] + raceLen, coords[1]], ', MINSTREL', fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(troubadour, (coords[0] + iconSize[0] + margin[0], coords[1] + raceclassTextSize),
                                  troubadour)  # add icon
                if cards[card, 9] == 'mage':
                    I1.text([coords[0] + raceLen, coords[1]], ', MAGE', fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(mage, (coords[0] + iconSize[0] + margin[0], coords[1] + raceclassTextSize),
                                  mage)  # add icon
                if cards[card, 9] == 'champion':
                    I1.text([coords[0] + raceLen, coords[1]], ', CHAMPION', fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(champion, (coords[0] + iconSize[0] + margin[0], coords[1] + raceclassTextSize),
                                  champion)  # add icon
                if cards[card, 9] == 'cultist':
                    I1.text([coords[0] + raceLen, coords[1]], ', CULTIST', fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(cultist, (coords[0] + iconSize[0] + margin[0], coords[1] + raceclassTextSize),
                                  cultist)  # add icon

                # Race of Questgiver
                spacing = (descriptionCoords[0] + font_raceclass.getsize('Questgiver:        ')[0],
                           descriptionCoords[1] + 10 - int(iconSize[1] * 0.25))
                shift = 15
                if cards[card, 10] == 'human':
                    I1.text([descriptionCoords[0], descriptionCoords[1] + shift], 'Questgiver: HUMAN  ',
                            fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(human, (spacing[0], spacing[1]), human)  # add icon
                if cards[card, 10] == 'dwarf':
                    I1.text([descriptionCoords[0], descriptionCoords[1] + shift], 'Questgiver: DWARF  ',
                            fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(dwarf, (spacing[0], spacing[1]), dwarf)  # add icon
                if cards[card, 10] == 'goblin':
                    I1.text([descriptionCoords[0], descriptionCoords[1] + shift], 'Questgiver: GOBLIN ',
                            fill=(10, 10, 10), font=font_raceclass)
                    cardPNG.paste(goblin, (spacing[0], spacing[1]), goblin)  # add icon
                if cards[card, 10] == 'elf':
                    I1.text([descriptionCoords[0], descriptionCoords[1] + shift], 'Questgiver: ELF ', fill=(10, 10, 10),
                            font=font_raceclass)
                    cardPNG.paste(elf, (spacing[0], spacing[1]), elf)  # add icon
                if cards[card, 10] == 'orc':
                    I1.text([descriptionCoords[0], descriptionCoords[1] + shift], 'Questgiver: ORC ', fill=(10, 10, 10),
                            font=font_raceclass)
                ...
                if cards[card, 4] != 'null':
                    I1.text(rage2Coords, '2-HERO REWARD: {}'.format(cards[card, 4]), fill=(10, 10, 10),
                            font=font_raceclass)
            ...
            if cards[card, 2] == 'module':
                if cards[card, 8] == 'extratable':
                    ...
                    for nt in range(len(text)):
                        I1.text(np.add(np.add(titleCoords, [130, 120]), [0, nt * titleSize * shiftFactor]), text[nt],
                                fill=(10, 10, 10), font=font_name)

                    # Description
                    text = textwrap.wrap("{}".format(cards[card, 4]), width=int(builderDescriptionWidth))
                    for nt in range(len(text)):
                        I1.text(np.add(descriptionCoords, [130, nt * heroDescriptionTextSize * shiftFactor]), text[nt],
                                fill=(10, 10, 10), font=font_herodescription)
                    ...
                    text = textwrap.wrap("{}".format(cards[card, 1]), width=int(nameWidth))
                    for nt in range(len(text)):
                        I1.text(np.add(titleCoords, [20, 10 + shift + nt * titleSize * shiftFactor]), text[nt],
                                fill=(10, 10, 10), font=font_name)

                    # Race Boost
                    yCoord = int(rage2Coords[1] - 0.5 * font_raceclass.getsize('HUMAN MODULE ')[1])
                    if cards[card, 5] == 'human':
                        I1.text([rage2Coords[0] + shift, yCoord + 50], 'HUMAN MODULE', fill=(10, 10, 10),
                                font=font_raceclass)
                        cardPNG.paste(human,
                                      (shift + raceCoords[0] + font_raceclass.getsize('HUMAN MODULE ')[0], yCoord),
                                      human)  # add icon
                    if cards[card, 5] == 'dwarf':
                        I1.text([rage2Coords[0] + shift, yCoord + 50], 'DWARF MODULE', fill=(10, 10, 10),
                                font=font_raceclass)
                        cardPNG.paste(dwarf,
                                      (shift + raceCoords[0] + font_raceclass.getsize('DWARF MODULE ')[0], yCoord),
                                      dwarf)  # add icon
                    if cards[card, 5] == 'goblin':
                        I1.text([rage2Coords[0] + shift, yCoord + 50], 'GOBLIN MODULE', fill=(10, 10, 10),
                                font=font_raceclass)
                        cardPNG.paste(goblin,
                                      (shift + raceCoords[0] + font_raceclass.getsize('GOBLIN MODULE ')[0], yCoord),
                                      goblin)  # add icon
                    if cards[card, 5] == 'orc':
                        I1.text([rage2Coords[0] + shift, yCoord + 50], 'ORC MODULE', fill=(10, 10, 10),
                                font=font_raceclass)
                        cardPNG.paste(orc, (shift + raceCoords[0] + font_raceclass.getsize('ORC MODULE ')[0], yCoord),
                                      orc)  # add icon
                    if cards[card, 5] == 'elf':
                        I1.text([rage2Coords[0] + shift, yCoord + 50], 'ELF MODULE', fill=(10, 10, 10),
                                font=font_raceclass)
                        cardPNG.paste(elf, (shift + raceCoords[0] + font_raceclass.getsize('ELF MODULE ')[0], yCoord),
                                      elf)  # add icon
                    ...
                    for nt in range(len(text)):
                        I1.text(
                            np.add(raceCoords, [shift + 20, 70 + shift + nt * heroDescriptionTextSize * shiftFactor]),
                            text[nt], fill=(10, 10, 10), font=fontBuilderDescription)
            ...
            if cards[card, 2] == 'Hero':
                ...
                I1 = ImageDraw.Draw(cardPNG)
                # TYPE
                I1.text([int((cardwidth - font_type.getsize('HERO')[0]) * 0.5), typeCoords[1]], 'HERO',
                        fill=(10, 10, 10), font=font_type)
                ...
                # Class
                if cards[card, 9] == 'warrior':
                    I1.text([raceCoords[0] + raceLen[0], raceCoords[1]], 'WARRIOR', fill=(10, 10, 10),
                            font=font_raceclass)
                    cardPNG.paste(warrior, (classCoords[0], classCoords[1] + raceclassTextSize), warrior)  # add icon
                if cards[card, 9] == 'bandit':
                    I1.text([raceCoords[0] + raceLen[0], raceCoords[1]], 'BANDIT', fill=(10, 10, 10),
                            font=font_raceclass)
                    cardPNG.paste(bandit, (classCoords[0], classCoords[1] + raceclassTextSize), bandit)  # add icon
                if cards[card, 9] == 'priest':
                    I1.text([raceCoords[0] + raceLen[0], raceCoords[1]], 'PRIEST', fill=(10, 10, 10),
                            font=font_raceclass)
                    cardPNG.paste(priest, (classCoords[0], classCoords[1] + raceclassTextSize), priest)  # add icon
                if cards[card, 9] == 'troubadour':
                    I1.text([raceCoords[0] + raceLen[0], raceCoords[1]], 'MINSTREL', fill=(10, 10, 10),
                            font=font_raceclass)
                    cardPNG.paste(troubadour, (classCoords[0], classCoords[1] + raceclassTextSize),
                                  troubadour)  # add icon
                if cards[card, 9] == 'mage':
                ...
                if cards[card, 9] == 'champion':
                    I1.text([raceCoords[0] + raceLen[0], raceCoords[1]], 'CHAMPION', fill=(10, 10, 10),
                            font=font_raceclass)
                    cardPNG.paste(champion, (classCoords[0], classCoords[1] + raceclassTextSize), champion)  # add icon
                if cards[card, 9] == 'cultist':
                    I1.text([raceCoords[0] + raceLen[0], raceCoords[1]], 'CULTIST', fill=(10, 10, 10),
                            font=font_raceclass)
                ...
                for na in range(len(text)):
                    I1.text(np.add(abilityCoords, [0, (na + 1) * heroDescriptionTextSize * shiftFactor]), text[na],
                            fill=(10, 10, 10), font=font_herodescription)
                ...
                for nr1 in range(len(text)):
                    I1.text(np.add(rage1Coords, [90, (nr1) * heroDescriptionTextSize * shiftFactor]), text[nr1],
                            fill=(10, 10, 10), font=font_herodescription)
            ...
            if cards[card, 2] == 'Event':
                ...
                for nt in range(len(text)):
                    I1.text(np.add(descriptionCoords, [0, nt * seasonsDescriptionTextSize * shiftFactor]), text[nt],
                            fill=(10, 10, 10), font=font_seasonsDescription)

                # number of visitors today
                shift = 65
                I1.text([raceCoords[0], raceCoords[1] + shift], 'NUMBER OF VISITORS TODAY: '.format(cards[card, 5]),
                        fill=(10, 10, 10), font=font_raceclass)
                I1.text([int((cardwidth - font_title.getsize('5')[0]) / 2), raceCoords[1] + shift + 50],
                        '{}    ({})'.format(cards[card, 5], cards[card, 10]), fill=(10, 10, 10), font=font_title)
                ...
                if cards[card, 8] != "null":
                    ...
                    for ri in range(len(affectList)):
                        ...
                        if affectList[ri] == 'human':
                            cardPNG.paste(human, (xCoord + font_raceclass.getsize('human   ')[0], yCoord),
                                          human)  # add icon
                        if affectList[ri] == 'dwarf':
                            cardPNG.paste(dwarf, (xCoord + font_raceclass.getsize('dwarf   ')[0], yCoord),
                                          dwarf)  # add icon
                        if affectList[ri] == 'goblin':
                            cardPNG.paste(goblin, (xCoord + font_raceclass.getsize('goblin   ')[0], yCoord),
                                          goblin)  # add icon
                        ...
                        if affectList[ri] == 'warrior':
                            cardPNG.paste(warrior, (xCoord + font_raceclass.getsize('warrior   ')[0], yCoord),
                                          warrior)  # add icon
                        if affectList[ri] == 'bandit':
                            cardPNG.paste(bandit, (xCoord + font_raceclass.getsize('bandit   ')[0], yCoord),
                                          bandit)  # add icon
                        if affectList[ri] == 'priest':
                            cardPNG.paste(priest, (xCoord + font_raceclass.getsize('priest   ')[0], yCoord),
                                          priest)  # add icon
                        if affectList[ri] == 'troubadour':
                            cardPNG.paste(troubadour, (xCoord + font_raceclass.getsize('troubadour   ')[0], yCoord),
                                          troubadour)  # add icon
                        if affectList[ri] == 'champion':
                            cardPNG.paste(champion, (xCoord + font_raceclass.getsize('champion   ')[0], yCoord),
                                          champion)  # add icon
                        if affectList[ri] == 'mage':
                            cardPNG.paste(mage, (xCoord + font_raceclass.getsize('mage   ')[0], yCoord),
                                          mage)  # add icon
                        if affectList[ri] == 'cultist':
                            cardPNG.paste(cultist, (xCoord + font_raceclass.getsize('cultist   ')[0], yCoord),
                                          cultist)  # add icon
            ...
            if cards[card, 2] == 'notable':
                ...
                if cards[card, 9] == 'mayor':
                    cardPNG.paste(mayorIcon, (notableIconCoords[0], notableIconCoords[1] + raceclassTextSize),
                                  mayorIcon)  # add ic
                if cards[card, 9] == 'constable':
                    cardPNG.paste(constableIcon, (notableIconCoords[0], notableIconCoords[1] + raceclassTextSize),
                                  constableIcon)  # add ic
                if cards[card, 9] == 'carpenter':
                    cardPNG.paste(carpenterIcon, (notableIconCoords[0], notableIconCoords[1] + raceclassTextSize),
                                  carpenterIcon)  # add ic
                if cards[card, 9] == 'auditor':
                    cardPNG.paste(auditorIcon, (notableIconCoords[0], notableIconCoords[1] + raceclassTextSize),
                                  auditorIcon)  # add ic
                if cards[card, 9] == 'entertainer':
                    cardPNG.paste(entertainerIcon, (notableIconCoords[0], notableIconCoords[1] + raceclassTextSize),
                                  entertainerIcon)  # add ic
                if cards[card, 9] == 'crier':
                    cardPNG.paste(crierIcon, (notableIconCoords[0], notableIconCoords[1] + raceclassTextSize),
                                  crierIcon)  # add ic
                if cards[card, 9] == 'temperance':
                    cardPNG.paste(temperanceIcon, (notableIconCoords[0], notableIconCoords[1] + raceclassTextSize),
                                  temperanceIcon)  # add ic
                if cards[card, 9] == 'envoy':
                    cardPNG.paste(envoyIcon, (notableIconCoords[0], notableIconCoords[1] + raceclassTextSize),
                                  envoyIcon)  # add ic
                if cards[card, 9] == 'recruiter':
                    cardPNG.paste(recruiterIcon, (notableIconCoords[0], notableIconCoords[1] + raceclassTextSize),
                                  recruiterIcon)  # add ic
                if cards[card, 9] == 'tough':
                    cardPNG.paste(toughIcon, (notableIconCoords[0], notableIconCoords[1] + raceclassTextSize),
                                  toughIcon)  # add ic
                if cards[card, 9] == 'nuisance':
                    cardPNG.paste(nuisanceIcon, (notableIconCoords[0], notableIconCoords[1] + raceclassTextSize),
                                  nuisanceIcon)  # add ic
                ...
                for na in range(len(text)):
                    I1.text(np.add(description2Coords, [0, (na + 1) * heroDescriptionTextSize * shiftFactor]), text[na],
                            fill=(10, 10, 10), font=font_herodescription)
    ...
    for i in range(nHeroes):
        ...
        if herodata[i][2] == 'human':
            ...
            templateCopy.paste(classrace_im[10].convert("RGBA"), racecoords,
                               classrace_im[10].convert("RGBA"))  # add icon

        elif herodata[i][2] == 'orc':
            templateCopy.paste(classrace_im[11].convert("RGBA"), racecoords,
                               classrace_im[11].convert("RGBA"))  # add icon

        elif herodata[i][2] == 'elf':
            templateCopy.paste(classrace_im[12].convert("RGBA"), racecoords,
                               classrace_im[12].convert("RGBA"))  # add icon

        elif herodata[i][2] == 'elemental':
            templateCopy.paste(classrace_im[13].convert("RGBA"), racecoords,
                               classrace_im[13].convert("RGBA"))  # add icon

        # Draw Class
        classcoords = [int(width * 0.1), int(height * 0.4)]
        if herodata[i][3] == 'warrior':
            templateCopy.paste(classrace_im[0].convert("RGBA"), classcoords,
                               classrace_im[0].convert("RGBA"))  # add icon
        if herodata[i][3] == 'bandit':
            templateCopy.paste(classrace_im[1].convert("RGBA"), classcoords,
                               classrace_im[1].convert("RGBA"))  # add icon
        if herodata[i][3] == 'priest':
            templateCopy.paste(classrace_im[2].convert("RGBA"), classcoords,
                               classrace_im[2].convert("RGBA"))  # add icon
        if herodata[i][3] == 'troubadour':
            templateCopy.paste(classrace_im[3].convert("RGBA"), classcoords,
                               classrace_im[3].convert("RGBA"))  # add icon
        if herodata[i][3] == 'mage':
            templateCopy.paste(classrace_im[4].convert("RGBA"), classcoords,
                               classrace_im[4].convert("RGBA"))  # add icon
        if herodata[i][3] == 'champion':
            templateCopy.paste(classrace_im[5].convert("RGBA"), classcoords,
                               classrace_im[5].convert("RGBA"))  # add icon
        if herodata[i][3] == 'cultist':
            templateCopy.paste(classrace_im[6].convert("RGBA"), classcoords,
                               classrace_im[6].convert("RGBA"))  # add icon
        ...
        for nt in range(len(text)):
            textheight = 30
            I1.text((int(width * 0.1), int(height * 0.55) + nt * textheight), "{}".format(text[nt]), fill=(10, 10, 10),
                    font=font_t)
